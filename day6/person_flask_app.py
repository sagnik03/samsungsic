...
persons = person_service.Person_service()

persons.oprs.create_table()
app = Flask(__name__)

@app.route('/persons',methods=['POST'])
def persons_create():
    body = request.get_json()
    new_person = persons.create_person(body['name'], body['gender'], body['dob'], body['location'])
    print(type(new_person))
    id = persons.insert_person(new_person)
    person = persons.search_person(id)
...
def persons_read_all():
    persons_list = persons.list_all_persons()
    persons_dict = []
...
def persons_update(id):
    body = request.get_json()
    old_peron_obj = persons.search_person(id)
